export type Source = {id:string; company:string; provider:'brave'|'greenhouse'|'lever'|'ashby'|'linkedin'|'jobicy'|'remoteok'|'arbeitnow'; token:string; careers?:string; checkedAt?:string; count?:number; error?:string};
export type Job = {id:string; source:string; company:string; title:string; location:string; url:string; linkedinUrl?:string; linkedinId?:string; origin?:'manual'|'linkedin'|'discovery'; description:string; department:string; salary:string; salaryMin?:number; salaryMax?:number; updatedAt?:string; postedAt?:string; prospect?:boolean; status:string; reason:string; coverLetter:string; firstSeen:string; lastSeen:string; active:number; verification?:'employer'|'unverified'; verifiedAt?:string; validThrough?:string; duplicateIds?:string[]; requisitionId?:string; applicationUrls?:string[]; discoveryUrl?:string; verificationCheckedAt?:string; verificationReason?:string};
...
export function locationSignal(job:Job){const remote=/\bremote\b|telecommut|work from home/i.test(job.location)&&!(/\bhybrid\b|\bon[- ]?site\b|not remote|no remote|remote (?:not|unavailable)/i.test(job.location));return {remote,possible:remote,label:remote?'Remote · check eligibility':'Check work arrangement'};}
...
export function scoreJob(j:Job,p:Profile,now=Date.now()){
 ...
 const officialProvider=officialListingProvider(j);const official=(j.verification==='employer'||(!j.verification&&Boolean(officialProvider)))&&Boolean(j.active);const discovered=(j.origin==='linkedin'||j.origin==='discovery')&&!official;
 qualitySignals.push({label:official?(j.origin?`Checked on ${officialProvider||'employer page'}`:'Present in employer ATS feed'):discovered?`${j.origin==='linkedin'?'LinkedIn':'Market'} discovery — employer listing not yet verified`:'Manually added — source not yet verified',points:official?50:discovered&&j.active?35:20});
 ...
 if(j.active&&age<=2)qualitySignals.push({label:'Listing observed within 48 hours (not its posting date)',points:15});else warnings.push('Needs a fresh source check');
