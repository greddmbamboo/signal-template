...
const actionSchema=z.discriminatedUnion('action',[
 z.object({action:z.literal('status'),id:z.string().max(250),status:z.enum(['inbox','applied','passed','flagged']),reason:z.string().max(500).optional()}),
...
export async function POST(request:Request){
 ...
 if(a.action==='status'){const result=await db.prepare('UPDATE jobs SET status = ?, reason = ? WHERE owner = ? AND id = ?').bind(a.status,a.reason||'',user.id,a.id).run();if(!result.meta.changes)return Response.json({error:'Listing not found.'},{status:404});}
 ...
 if(a.action==='resetSearch')await db.prepare("DELETE FROM jobs WHERE owner = ? AND source != 'manual' AND status != 'applied'").bind(user.id).run();
