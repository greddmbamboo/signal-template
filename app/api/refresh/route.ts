...
export async function POST(request:Request){
 ...
 try{const profile=await getProfile(user.id);const found=['jobicy','remoteok','arbeitnow'].includes(source.provider)?await fetchDiscovery(source,profile):(await fetchSource(source)).filter(job=>matchesLocationPreference(job,profile)&&matchesTargetRole(job.title,profile.roles));return Response.json({source:await saveFeed(user.id,source,found)})}catch(e){console.error('source check failed',source.id,e);const message=e instanceof Error&&/returned HTTP|too large/.test(e.message)?e.message:'The feed could not be reached or read. Existing listings were kept.';await recordSourceError(user.id,source,message);return Response.json({error:`${source.company}: ${message}`},{status:502});}
